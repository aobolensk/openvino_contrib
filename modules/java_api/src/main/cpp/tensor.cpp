...
Java_org_intel_openvino_Tensor_TensorCArray(JNIEnv* env, jobject, jint type, jintArray shape, jlong matDataAddr) {
    JNI_METHOD(
        "TensorCArray", auto input_type = get_ov_type(type); Shape input_shape = jintArrayToVector(env, shape);
...
JNIEXPORT jlong JNICALL Java_org_intel_openvino_Tensor_TensorFloat(JNIEnv* env,
                                                                   jobject,
                                                                   jintArray shape,
                                                                   jfloatArray data) {
    JNI_METHOD("TensorFloat", Shape input_shape = jintArrayToVector(env, shape);
...
JNIEXPORT jlong JNICALL Java_org_intel_openvino_Tensor_TensorInt(JNIEnv* env,
                                                                 jobject,
                                                                 jintArray shape,
                                                                 jintArray data) {
    JNI_METHOD("TensorInt", Shape input_shape = jintArrayToVector(env, shape);
...
JNIEXPORT jlong JNICALL Java_org_intel_openvino_Tensor_TensorLong(JNIEnv* env,
                                                                  jobject,
                                                                  jintArray shape,
                                                                  jlongArray data) {
    JNI_METHOD("TensorLong", Shape input_shape = jintArrayToVector(env, shape);
...
JNIEXPORT jint JNICALL Java_org_intel_openvino_Tensor_GetSize(JNIEnv* env, jobject, jlong addr) {
    JNI_METHOD("GetSize", Tensor* ov_tensor = (Tensor*)addr; return (jint)ov_tensor->get_size();)
    return 0;
}

JNIEXPORT jintArray JNICALL Java_org_intel_openvino_Tensor_GetShape(JNIEnv* env, jobject, jlong addr) {
    JNI_METHOD(
        "GetShape", Tensor* ov_tensor = (Tensor*)addr; Shape shape = ov_tensor->get_shape();

        jintArray result = env->NewIntArray(shape.size());
        if (!result) { throw std::runtime_error("Out of memory!"); } jint* arr =
            env->GetIntArrayElements(result, nullptr);

        for (int i = 0; i < shape.size(); ++i) arr[i] = shape[i];
...
JNIEXPORT jfloatArray JNICALL Java_org_intel_openvino_Tensor_asFloat(JNIEnv* env, jobject, jlong addr) {
    JNI_METHOD(
        "asFloat", Tensor* ov_tensor = (Tensor*)addr;

        size_t size = ov_tensor->get_size();
        const float* data = ov_tensor->data<const float>();

        jfloatArray result = env->NewFloatArray(size);
        if (!result) { throw std::runtime_error("Out of memory!"); } jfloat* arr =
            env->GetFloatArrayElements(result, nullptr);
...
JNIEXPORT jintArray JNICALL Java_org_intel_openvino_Tensor_asInt(JNIEnv* env, jobject, jlong addr) {
    JNI_METHOD(
        "asInt", Tensor* ov_tensor = (Tensor*)addr;
